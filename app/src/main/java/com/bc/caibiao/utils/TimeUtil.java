...
public class TimeUtil {
    ...
    public final static String FORMAT_YEAR_MONTH = "yyyy年MM月";
    ...
    public static String getTimeAfter(long timestamp) {
        try {
            long currentTime = System.currentTimeMillis();
            long timeGap = (timestamp * 1000 - currentTime) / 1000;// 与现在时间相差秒数
            String timeStr = null;
            if (timeGap > YEAR) {
                timeStr = getFormatTimeFromTimestamp(timestamp * 1000, FORMAT_DATE_TIME);
            } else if (timeGap > THREEDAYS && timeGap < YEAR) {// 3天以上显示X月X日 X:X
                timeStr = getFormatTimeFromTimestamp(timestamp * 1000, "MM-dd");
            } else if (timeGap > DAY && timeGap < THREEDAYS) {// 1天以上3天以下
     ...
     * @return
     */
    public static String getFormatTimeFromTimestamp(int timestamp, String format) {
        return getFormatTimeFromTimestamp(timestamp * 1000l, format);
    }
    ...
    public static String getDesTimeStamp(String currentDate){

        Date currentDay = getTimeFromString(currentDate,"yyyy-MM-dd");

        return String.valueOf(currentDay.getTime()/1000);
