 ...
 * @author Sai 支持自动翻页
 */
public class ConvenientBanner<T> extends LinearLayout implements NetworkImageHolderView.ItemClickListener {
	...
	public ConvenientBanner setPages(CBViewHolderCreator holderCreator, List<T> datas, String header) {
		this.mDatas = datas;
		this.holderCreator = holderCreator;
		pageAdapter = new CBPageAdapter(holderCreator, mDatas, header);
		pageAdapter.setItemClickListener(this);
...
//			viewPager.setPageTransformer(true, (PageTransformer) Class.forName(pkName + ".transforms." + transformer.getClassName()).newInstance());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return this;
	...
	public void onItemClick() {
		if (pageChangeListener != null) {
			try {
//				String skipURL = ((Banner) mDatas.get(pageChangeListener.getmIndex())).url;
//				String title = ((Banner) mDatas.get(pageChangeListener.getmIndex())).title;
//
//				Intent aIntent = new Intent(mContext, SimpleOutlinkActivity.class);
//				aIntent.putExtra("openUrl", skipURL);
